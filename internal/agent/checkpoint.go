...
func (c *Checkpointer) Checkpoint(ctx context.Context, t Target) (*Result, error) {
	...
	// (5) Resume the SOURCE (non-destructive checkpoint): restore control state via
	// cuda-checkpoint, then remap the GPU data buffers — recreate physical memory,
	// map it back to the preserved virtual addresses, and copy the host-resident
	// data back to the device — so the source workload keeps running.
	// (tar -> new container restore is a separate, user-triggered op needing CRI-O.)
	if err := c.cudaToggle(ctx, t.HostPID); err != nil {
	...
	klog.Infof("step 5/5 done: checkpoint stored at %s; source resumed (remapped); took %s", stored, time.Since(start))
