...
`schedule`를 주면 **Go duration**("5m","1h") 또는 **cron**("0 */2 * * *","@hourly")으로
반복 체크포인트가 됩니다. 각 replica의 자식이 그 주기로 재체크포인트되고, orchestrator는
30초마다 워크로드를 **재-resolve**해 스케일업/교체로 생긴 **새 replica에도 자동으로 자식을
붙입니다**.
```yaml
spec:
  workloadRef: { kind: Deployment, namespace: default, name: gpu-infer }
  schedule: "0 */2 * * *"   # 2시간마다 (cron). "30m" 처럼 duration도 가능
  storage: { type: nfs, endpoint: "10.178.0.14", source: "10.178.0.14:/mnt/nfs", path: /mnt/nfs, subPath: gcr }
```
...
- **스케줄**: `schedule`는 Go duration과 cron을 모두 지원합니다(node-agent에서 파싱).
  recurring일 때 orchestrator가 주기적으로 Pod를 재-resolve해 새 replica를 자동 포함합니다.
  one-shot(schedule 없음)은 최초 resolve된 Pod 집합만 대상으로 하며 이후 추가 replica는
  포함하지 않습니다.
