...
2. **NVIDIA 드라이버 550** (cuda-checkpoint 지원, CUDA 12.4 ABI) → *여기서 재부팅 요청*
...
GPU 워커에 vllm 대신 **PyTorch(CUDA 12.1) Pod** 를 띄웁니다 (드라이버 550과 호환,
약 4GB GPU 텐서 할당 후 대기). 인터셉터를 LD_PRELOAD 합니다.

```bash
kubectl apply -f deploy/sample-pod-pytorch.yaml
kubectl get pod cuda-gcr-pod -o wide -w     # Running 확인
kubectl logs cuda-gcr-pod | head            # "GPU tensor allocated ..." 확인
```
